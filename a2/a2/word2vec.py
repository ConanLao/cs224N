...
def naiveSoftmaxLossAndGradient(
    ...
    Uv = outsideVectors.dot(centerWordVec)
    y_hat = softmax(Uv)
...
def negSamplingLossAndGradient(
    ...
    loss = -np.log(sigmoid(v_c.dot(u_o))) + -np.sum(np.log(sigmoid(-U[negSampleWordIndices].dot(v_c))))
    gradCenterVec = (sigmoid(z_o) - 1) * u_o
    gradOutsideVecs = np.zeros(shape=outsideVectors.shape)
    
    gradOutsideVecs[outsideWordIdx] = ((sigmoid(z_o) - 1) * v_c).reshape(1, -1)

    for k in negSampleWordIndices:
        z = U[k].dot(v_c)
        a = sigmoid(z)
        gradOutsideVecs[k] += a*v_c
        gradCenterVec += a*U[k]
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for o in outsideWords:
        d_loss, d_gradCenterVec, d_gradOutsideVectors = word2vecLossAndGradient(v_c, word2Ind[o], U, dataset)
        loss += d_loss
        gradCenterVecs[word2Ind[currentCenterWord]] += d_gradCenterVec
        gradOutsideVectors += d_gradOutsideVectors
    ### END YOUR CODE
