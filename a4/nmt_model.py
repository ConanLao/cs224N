...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.encoder = nn.LSTM(
            input_size=embed_size, hidden_size=self.hidden_size, bias=True, bidirectional=True)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        X = pack_padded_sequence(X, lengths=torch.Tensor(source_lengths), batch_first=False) 
        enc_hiddens, (last_hidden, last_cell) = self.encoder(X) # enc_hiddens.shape = [seq_len, batch_size, 2 * features]
        enc_hiddens, _out_len = pad_packed_sequence(enc_hiddens, batch_first=True)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        # 1
        enc_hiddens_proj = self.att_projection(enc_hiddens) # the linear transformation is applied on the last dimension only
        ...
        for Y_t in torch.split(Y, 1, dim=0):
            ...
            Y_t_bar = torch.cat((Y_t, o_prev), 1) # Y_t : [b, e], o_prev : [b, h]
            dec_state, o_t, _ = self.step(Y_t_bar , dec_state, enc_hiddens, enc_hiddens_proj, enc_masks)
            combined_outputs.append(o_t)
            o_prev = o_t

        # 4
        combined_outputs = torch.stack(combined_outputs, dim=0) # combined_outputs : [tgt, b, h]
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        # 3
        # This step seems to be why we need to transpose the result of encoder.
        # i.e. we want b to be dim=0 in enc_hiddens_proj.
